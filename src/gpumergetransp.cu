...
__global__ void min_min_sorted(float* machines, uint* task_index, float* completion_times, int* task_map,
	...
	while (task_map[task_index[j*m + i]] != -1) {
		j++;
	}
	machine_current_index[i] = j;

	s_comp_times[idx] = completion_times[i] + machines[j*m + i];
	s_ind_max[idx] = i;
...
__global__ void blocks_reduction(float* completion_times, int* task_map,
		uint* machine_current_index, float* reduced_times, uint* reduced_indexes, int m, int t) {
...
int main(int argc, char** argv) {
	...
	*/
	...
	dim3 dimGridTransp(t/TILE_DIM, m/TILE_DIM, 1);
	dim3 dimBlockTransp(TILE_DIM, BLOCK_ROWS, 1);

	if (t % TILE_DIM || m % TILE_DIM) {
	...
	transposeNoBankConflicts<<<dimGrid, dimBlock>>>(d_machines_out, d_machines);
	transposeNoBankConflicts<<<dimGrid, dimBlock>>>(d_task_index_out, d_task_index);

	for(int k = 0; k < t; k++) {

		min_min_sorted<<<dimGrid, dimBlock, block * sizeof(float) + block * sizeof(int) >>>(d_machines_out, d_task_index_out,
						d_completion_times,	d_task_map, d_machine_cur_index, reduced_times, reduced_indexes, m, t);

		blocks_reduction<<<	1, grid, grid * sizeof(float) + grid * sizeof(int) >>>(d_completion_times,	d_task_map, d_machine_cur_index, reduced_times, reduced_indexes, m, t);
	}
